...
class TerrainConfig:
    ...
    normal_ground_clearance: float = 0.13  # Độ cao gầm bình thường (m) = 13cm
    
    # Camera mounting
    # Góc nghiêng 15° so với phương NGANG (horizon)
    # = Camera gần như nhìn thẳng, hơi cúi xuống
    camera_height: float = 0.20         # Chiều cao camera so với mặt đất (m)
    camera_tilt_angle: float = 15.0     # Góc so với phương ngang (độ), dương = cúi xuống
    camera_vfov: float = 58.0           # Vertical FOV của camera (độ) - RealSense D435
    
    # Ceiling detection zones (ratio of frame height)
    # Với góc 15°, top frame nhìn lên ~14° so với horizon → có thể thấy trần
    ...
    # Ground obstacle detection zones
    # Với góc 15°, bottom frame nhìn xuống ~44° → thấy mặt đất gần
    ground_zone_top: float = 0.55       # Từ 55% frame height
    ground_zone_bottom: float = 0.90    # Đến 90%
    ground_baseline_distance: float = 1.0  # Khoảng cách baseline mặt đất (m)
    obstacle_threshold: float = 0.08    # Chênh lệch depth để coi là obstacle (m)
    max_step_height: float = 0.05       # Chiều cao tối đa có thể bước qua (m)
    
    # Processing
    depth_min_valid: float = 0.1
    depth_max_valid: float = 5.0
...
class TerrainAnalyzer:
    ...
    def _analyze_ground(
        ...
        """Phân tích vùng mặt đất để tìm chướng ngại vật."""
        # Extract ground zone
        ...
        # Baseline: khoảng cách mặt đất bình thường (median)
        baseline = float(np.median(valid_depths))
        
        # Tìm điểm gần hơn baseline nhiều → chướng ngại vật
        obstacle_mask = valid_depths < (baseline - self.config.obstacle_threshold)
        ...
        # Ước tính chiều cao chướng ngại vật
        # 
        # Camera setup: nhìn về phía trước, nghiêng 15° so với phương NGANG
        # 
        ...
        #           Mặt đất
        #
        # Với camera nghiêng nhẹ (15° so với horizon):
        # - Góc tia nhìn đến mặt đất (bottom frame) ≈ 15° + FOV/2 ≈ 44°
        # - Góc tia nhìn đến vật cũng trong vùng này
        #
        # Công thức: h = d_ground × sin(θ_ground) - d_obstacle × sin(θ_obstacle)
        # Đơn giản hóa với góc trung bình của ground zone:
        
        depth_diff = baseline - obstacle_distance
        
        # Góc trung bình của ground zone so với horizon
        # Center của frame nghiêng 15°, bottom frame nghiêng thêm ~FOV/2
        avg_ground_angle = self.config.camera_tilt_angle + self.config.camera_vfov * 0.25
        angle_rad = np.radians(avg_ground_angle)
        
        # Chiều cao = chênh lệch depth × sin(góc)
    ...
    def _determine_action(
        ...
        # Priority: Ceiling first (safety), then ground obstacles
        
        # Case 1: Ceiling too low
        if ceiling['detected'] and not ceiling['clearance_ok']:
        ...
        # Case 2: Ceiling low but passable
        ...
        # Case 3: Ground obstacle that can be stepped over
            ...
            needed_clearance = ground['height'] + 0.02  # Buffer 2cm
            needed_clearance = min(needed_clearance, self.config.max_ground_clearance)
            
            result.action = ClearanceAction.RAISE
            result.recommended_height = needed_clearance
            result.message = f"Chướng ngại {ground['height']*100:.1f}cm - NÂNG GẦM lên {needed_clearance*100:.0f}cm"
            result.confidence = 0.85
        
        # Case 4: Ground obstacle too high
