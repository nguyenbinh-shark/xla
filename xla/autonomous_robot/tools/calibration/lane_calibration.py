...
class LaneCalibrationTool:
    ...
    def _create_display(self, results: dict, params: dict) -> np.ndarray:
        ...
        # Add parameter info
        info_h = 100
        info_panel = np.zeros((info_h, display.shape[1], 3), dtype=np.uint8)
        
        info_lines = [
            f"ROI: Top Y={params['roi_top_y']}% X=[{params['roi_top_left_x']}%-{params['roi_top_right_x']}%]",
            f"     Bot Y={params['roi_bottom_y']}% X=[{params['roi_bottom_left_x']}%-{params['roi_bottom_right_x']}%]",
            f"Thresh={params['black_threshold']} Morph={params['morph_kernel']} Canny=[{params['canny_low']}-{params['canny_high']}]",
            f"Hough: T={params['hough_threshold']} MinLen={params['hough_min_length']} Gap={params['hough_max_gap']}",
        ]
        
        for i, line in enumerate(info_lines):
            cv2.putText(info_panel, line, (10, 20 + i * 22),
        ...
        cv2.putText(display, instr, (display.shape[1] - 300, display.shape[0] - 10),
